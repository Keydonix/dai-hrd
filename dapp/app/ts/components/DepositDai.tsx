...
import { decimalStringToBigintDai } from '../library/conversion';
...
export function DepositDai(model: DepositDaiModel) {
	...
	let attodaiToConvert = decimalStringToBigintDai(daiToConvert)
	...
	return <article className='panel'>
		...
		{model.depositState === 'approved' &&
			<>
				<input style={{ margin: '5px' }} type='number' placeholder='Amount of DAI to convert.' onChange={event => setDaiToConvert(event.target.value)} value={daiToConvert} />
				{attodaiToConvert &&
					<button onClick={() => attodaiToConvert ? model.deposit(attodaiToConvert) : undefined }>Convert</button>
				}
			</>
		}
