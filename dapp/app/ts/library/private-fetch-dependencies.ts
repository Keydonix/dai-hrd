...
export class PrivateFetchDependencies implements Dependencies {
	...
	public constructor(jsonRpcEndpoint: string) {
		this.rpc = new FetchJsonRpc(jsonRpcEndpoint, window.fetch.bind(window), {
			gasPriceInAttoethProvider: async () => 10n**9n,
			gasLimitProvider: async (transaction, estimator) => await estimator(transaction) + 55_000n,
			addressProvider: async () => this.address,
			signatureProvider: async (message: Bytes): Promise<{ r: bigint, s: bigint, yParity: 'even'|'odd' }> => {
